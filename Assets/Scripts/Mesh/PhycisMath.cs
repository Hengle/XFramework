...
namespace XDEDZL.Mesh
{
    ...
    public class PhyscisMath
    {
        ...
        private static float GetDelta(float a, float b, float c)
        {
            return b * b - 4 * a * c;
        }
        ...
        /// <param name="theta">旋转角度 默认为90度</param>
        /// <returns></returns>
        ...
        /// 获取两点间向上的竖直垂直向量
        /// </summary>
        /// 将原向量和垂直向量都投影到水平面上时，向量方向应该一致，所以两个向量的x，z应相等，通过公式可求得垂直向量的y
        /// <param name="_start"> 起始点 </param>
        /// <param name="_end"> 终止点 </param>
        /// <returns></returns>
        public static Vector3 GetVerticalDir(Vector3 _start, Vector3 _end)
        {
            ...
            // 获取两点间向量的垂直向量
            Vector3 dirUp = new Vector3(vector.x, (vector.x * vector.x + vector.z * vector.z) / -vector.y, vector.z).normalized;
            //由于是取向上的向量，所以当y<0时，对结果取反
            if (dirUp.y < 0) 
            {
                dirUp = -dirUp;
            }
        ...
        public static Vector3[] GetVertices(Vector3 fromPos, float radius, int angle)
        {
            ...
            for (int theta = 1; theta <= angle; theta += 1)
            {
                for (int alpha = 0; alpha < 360; alpha += 1)
                {
                    ...
                    // 射线判断碰撞是否需要改变顶点位置
                    if (Physics.Raycast(fromPos, direction, out hitInfo, radius, LayerMask.GetMask("Terrain")))
                    {
                        point = hitInfo.point;
                        vertices.Add(point);
                    }
