    ...
    public class CameraOpr : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (m_isPosChanged)
            {
                ...
                m_mainCamera.transform.Translate(m_detalVector,Terrain.activeTerrain.transform);
        ...
        void CameraRotate()
        {
            ...
            {
                //镜头旋转可以360度任意旋转 
                m_mainCamera.transform.RotateAround(m_mainCamera.transform.position, Vector3.up, Input.GetAxis("Mouse X") * m_rotateSpeed * Time.deltaTime);

                //镜头的俯仰在（-10，60）之间
                float eulerAngles_x = m_mainCamera.transform.eulerAngles.x;
                ...
                {
                    m_mainCamera.transform.RotateAround(m_mainCamera.transform.position, m_mainCamera.transform.right, -Input.GetAxis("Mouse Y") * m_rotateSpeed * Time.deltaTime);
                }
            }
            else if (m_isVerticalChange)
            {
                ...
                //镜头角度随高度变化函数
                //-0.00001748251748251755*x*x+0.06561771561771562*x-1.3053613053613053 
                //m_targetAngel = -0.0000174825f * camera2surfaceHeight * camera2surfaceHeight + 0.0656177156f * camera2surfaceHeight - 1.3053613f;
                ...
                if (0 < m_targetAngel && m_targetAngel < 60)
                {
                    ...
                    m_mainCamera.transform.RotateAround(m_mainCamera.transform.position, m_mainCamera.transform.right, detalAngel * Time.deltaTime);
                }
            }
        }
        ...
        void CameraHorizontal()
        {
            ...
            {
                m_velocity -= m_mainCamera.transform.right;
                m_isPosChanged = true;
            }
        ...
        void CameraLongitudinal()
        {
            if (Input.GetKey(KeyCode.W)) //W键前移
            {
                ...
                m_velocity += Vector3.Normalize(m_mainCamera.transform.forward + m_mainCamera.transform.up * (Mathf.Tan(m_mainCamera.transform.eulerAngles.x * Mathf.Deg2Rad)));
                m_isPosChanged = true;
            }
        ...
        void CameraVertical()
        {
            ...
            if (Input.GetKey(KeyCode.E)) //E键向下移
            {
                m_velocity -= Vector3.up;
                m_isPosChanged = true;
                m_isVerticalChange = true;
            }
            if (Input.GetKey(KeyCode.Q)) //Q键向上移
            {
                m_velocity += Vector3.up;
                m_isPosChanged = true;
                m_isVerticalChange = true;
        ...
        void CameraHeightLimit()
        {
            ...
            {
                if (Physics.Raycast(cameraPos, Vector3.down, out m_raycastHit, 320f, LayerMask.GetMask("Terrain","Table")))
                {
                    ...
                    Vector3 cameraVector = m_mainCamera.transform.forward;
                    cameraVector.y = 0;
        ...
        void CameraPushAndPull()
        {
            ...
            else if (srcollWheel != 0)
            {
                ...
                {
                    m_mouseVector = m_mainCamera.ScreenPointToRay(Input.mousePosition).direction;   //获取相机到鼠标点的方向向量
                ...
                {
                    m_mouseVector = m_mainCamera.transform.forward;
